...
module.exports = async function (req, res) {
    ...
    if (skin == false){
        return res.status(200).json({ success : false, code: 2, error : "Скин не существует" })
    }
    ...
    if (isPurchased == false){
        return res.status(200).json({ success : false, code: 2, error : "Вы не покупали данный скин" })
    } else {
        // Make transaction
