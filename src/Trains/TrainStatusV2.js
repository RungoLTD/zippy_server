...
module.exports = async function (req, res) {
    ...
    let result = await db.mysqlQuery('SELECT *, UNIX_TIMESTAMP(start_day) as start_day FROM user_trains WHERE user_id = ? AND end_day IS NULL ORDER BY start_day DESC LIMIT 1', [
        user.id,
    ]);
    var train = null;
    ...
    if (result != false) {
        ...
        train = { ...trains.fullFormatted[result.train_section].trains[result.train_index] };

        // Получаем результат пользователя
        let userStatistics = await db.mysqlQueryArray('SELECT *, UNIX_TIMESTAMP(created) as created FROM statistics WHERE user_id = ? AND created >= ?', [
            user.id,
            result.start_day,
        ]);
        ...
        trainProgramm.forEach((train) => {
            ...
            if (train.type != declarations.Type.Rest) {
                ...
                trainStatistics = {
                    ...
                    train: {
                        ...train,
                        typeLabel: declarations.getTypeDescription(train.type),
                    },
    ...
    if (result) {
        ...
        if (rules.rule_1_sended_date == null) {
            ...
            if (isTrain3week && isSkippedLessThan2Days && isPositiveMoreThan80Percent && isMood2CountMoreThan3) {
                ...
                ChatService(
                    user.id,
                    'Вау! Я проанализировал твою программу тренировок, у тебя отличные результаты! Как думаешь, возможно ты готов к небольшому усложнению программы? ….',
                    'train_1_1',
                );
                return;
            }
            ...
            if (isTrain1week && isSkippedLessThan2Days) {
                ...
                ChatService(
                    user.id,
                    'Я проанализировал твою программу тренировок. Скажи, что тебя волнует, давай попробуем исправить? Я предлагаю тебе выбрать другую программу или облегчить текущую тренировку. Что скажешь?',
                    'train_2_1',
                );
                return;
            }
            ...
            if (isTrain2week && isSkippedLessThan2Days && isNegativeMoreThan50Percent && isMood2CountMoreThan4) {
                ...
                ChatService(
                    user.id,
                    'Я проанализировал твою программу тренировок. Твои показатели настроения после тренировок говорят о том, что возможно тебе немного сложно тренироваться с такой нагрузкой? Помни, что это абсолютно нормально. Я предла-гаю тебе облегчить текущую программу тренировок. Что скажешь?',
                    'train_2_1',
                );
                return;
            }
            ...
            let moodCount = await db.mysqlQuery('SELECT COUNT(mood1) as mood1count, COUNT(mood2) as mood2count FROM user_mood_log WHERE user_id = ? AND created > ?', [
                user.id,
                result.start_day,
            ]);
            ...
            if (isTrain4week && isSkippedLessThan2Days && isNegativeMoreThan60Percent && isMood2CountMoreThan4) {
                ...
                ChatService(
                    user.id,
                    'Оооо! Я проанализировал твою программу тренировок, у тебя плохие результаты! Возможно тебе следует отдохнуть, 1 неделю….И возобновить тренировки! ',
                    'train_2_1',
                );
                return;
            }
